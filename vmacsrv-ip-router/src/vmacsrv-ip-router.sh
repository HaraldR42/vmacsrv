...
NET_ETHOUDP=$(ip    -json addr show dev "$ETHOUDP_IF"    | jq -r 'first(.[0].addr_info[] | select(.family=="inet") | .local+"/"+(.prefixlen|tostring))')
...
function start() {
    ...
    ipt_add FORWARD -s "$NET_VINTAGELAN" -d "$NET_ETHOUDP"  -j ACCEPT
    ipt_add FORWARD -s "$NET_ETHOUDP"  -d "$NET_VINTAGELAN" -j ACCEPT
    ...
    ipt_add FORWARD -s "$NET_ETHOUDP"  -o "$LAN_IF" -j ACCEPT

    # ── Masquerade: internal -> external, but NOT to the peer internal net ─────
    ipt_add_t nat POSTROUTING -s "$NET_VINTAGELAN" ! -d "$NET_ETHOUDP"  -o "$LAN_IF" -j MASQUERADE
    ipt_add_t nat POSTROUTING -s "$NET_ETHOUDP"  ! -d "$NET_VINTAGELAN" -o "$LAN_IF" -j MASQUERADE
}


function stop() {
    ipt_del FORWARD -s "$NET_VINTAGELAN" -d "$NET_ETHOUDP"  -j ACCEPT
    ipt_del FORWARD -s "$NET_ETHOUDP"  -d "$NET_VINTAGELAN" -j ACCEPT

    ipt_del FORWARD -i "$LAN_IF" \
        -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT

    ipt_del FORWARD -s "$NET_VINTAGELAN" -o "$LAN_IF" -j ACCEPT
    ipt_del FORWARD -s "$NET_ETHOUDP"  -o "$LAN_IF" -j ACCEPT

    ipt_del_t nat POSTROUTING -s "$NET_VINTAGELAN" ! -d "$NET_ETHOUDP"  -o "$LAN_IF" -j MASQUERADE
    ipt_del_t nat POSTROUTING -s "$NET_ETHOUDP"  ! -d "$NET_VINTAGELAN" -o "$LAN_IF" -j MASQUERADE
}

echo "IP router container starting"
echo "  LAN_IF=$LAN_IF"
echo "  VINTAGELAN_IF=$VINTAGELAN_IF ($NET_VINTAGELAN)"
echo "  ETHOUDP_IF=$ETHOUDP_IF ($NET_ETHOUDP)"
